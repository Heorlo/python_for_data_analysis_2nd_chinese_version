...
> 注意直接输入data，与print(data)输出的结果不同
...
> jupyter的好处是在程序运行完之后你可以和结果进行交互，而不是和IDE一样运行完啥也没有了
...
IPython中特殊的命令（Python中没有）被称作“魔术”命令。这些命令可以使普通任务更便捷，更容易控制IPython系统。魔术命令是在指令前添加百分号%前缀。例如，可以用`%timeit`（这个命令后面会详谈）测量任何Python语句
...
> 说的有点绕口了，其实还是函数调用的时候，如果改变形参，实参也会改变，因为形参是对实参的引用，
...
> 分为引用和对象两种，编写程序的时候操作的是引用？通过引用操作对象，可以先后指向不同类型的对象
...
> 1. list是复制不是引用
> 2. == 与is 不同， ==判断值，is判断是不是引用同一个Python对象
...
> b''代表，是字符串的字节表示，在Python 3及以上版本中，Unicode是一级的字符串类型，果你知道一个字节对象的Unicode编码，用`decode`方法可以解码，即由字节decode得到对应编码的字符串
...
> 这个写法也很新鲜，其实就是迭代中取得的单个元素如果也是集合，可以这样取得元素内的值
>
> ```python
> x = [(1,2,3),(1,2,3),(1,2,3)]
> for a,b,c in x:
